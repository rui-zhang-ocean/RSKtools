...
* Version 3.4.0 (2020-02-14)
   ...
   - All RSKtools functions now require name-value pair format for input arguments except RSK structure and filename.
   - More derivation functions now support seawater library, including `RSKderivesalinity`, `RSKderivedepth`, `RSKderivebuoyancy`, `RSKderivetheta` and `RSKderivesigma`.
   - New function `RSKderivesoundspeed` to derive speed of sound in seawater.
   - New function `CSV2RSK` to read csv file into rsk structure in MATLAB
   - New function `RSKsettings` and `RSKdefaultsettings` to set up global parameters for RSKtools.
   - New function `RSKprintchannels` to display channel names and units in MATLAB command window.
   - New function `RSKerror` and `RSKwarning` to better handle error and warning message.
   - Fixed bug that `RSKderiveO2` can’t recognize saturation/concentration channel correctly
   - Fixed bug that `RSKgenerate2D` has to specify direction when only upcast is available.
   - Fixed bug that `RSKreaddata` can’t recognize derived and hidden channel correctly
   - Export functions now adjust path separator to be compatible cross platforms.
   - `RSK2RSK` now uses millisecond for sampling period unit.
   - Subfolder `Utilities` is renamed to `private`, which do not need users to add to MATLAB pathway anymore.
   - `Utilites/loadconstants` is removed.
   - `RSKreadata` can handle the case when file type is not available.
