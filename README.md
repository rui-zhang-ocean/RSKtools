...
Current stable version: 2.3.0 (2018-05-09)
...
* Version 2.3.0 (2018-05-09)
	...
	- New function `RSKaddmetadata` for adding metadata for specified profile.
	- `RSKopen` could read annotations from Ruskin, including latitude, longitude, comment and description by calling `RSKreadannotations`.
	- `RSKreadprofiles` parses latitude, longitude, comment and description into data structure.
	- Fixed bug that `RSKplot2D` not aligning profile with correct time stamp. 
	...
	- `direction` optional input added to `RSK2CSV`.
	- `RSK2CSV` will parse metadata if exists.
	- `showcast` option added in `RSKplotdata` to show detected cast directions on pressure channel.
